...
import { useSipCallSession } from 'dashboard/composables/useSipCallSession';
...
<template>
  <div
    ...
    class="fixed ltr:right-4 rtl:left-4 bottom-36 z-50 flex flex-col gap-2 w-72"
  >
    <div
      v-if="callError"
    ...
    >
      <div
        ...
        :class="{ 'animate-pulse': isOutboundPending }"
      >
        <Avatar
      ...
      <div class="flex-1 min-w-0">
        ...
        <p
          class="text-sm"
          :class="
            isOutboundPending ? 'text-n-slate-11' : 'font-mono text-n-teal-9'
          "
        >
          <template v-if="isOutboundCalling">
            {{ t('SIP_CALL.CALLING') }}
          </template>
      ...
      <div class="flex shrink-0 gap-2">
        <button
          ...
          "
          :title="isMuted ? t('SIP_CALL.UNMUTE') : t('SIP_CALL.MUTE')"
          @click="toggleMute"
        >
          <i
